...
mysqli_select_db("delicias", $cn);


if(@$_GET['action'] == 'listFotos'){
	
	$query = mysqli_query("SELECT * FROM publicar ORDER BY id_foto DESC", $cn);
	...
	$query = mysqli_query("DELETE FROM publicar WHERE id_foto = '".$_GET['id']."'", $cn);
	...
	if(in_array($type, $types)){
		...
		if(isset($_FILES['image'])){
			...
			if(move_uploaded_file($temp, $destino.$nombre))
			{
				...
				$query = mysqli_query("insert into publicar(nombre_foto,ubicacion) values('$nombre','$ubicacion')");
				$ID = mysqli_insert_id();
			}
